...
namespace Sass {
  ...
  namespace Functions {
    ...
    Signature print_sig = "print($msg)";
    BUILT_IN(print)
    {
      String_Constant* msg = ARG("$msg", String_Constant);
      cout << path << ":" << line << ": " << msg->value() << endl;
      return new (ctx.mem) String_Constant(path, line, msg->value());
    }


    Signature custom_func_sig = "__custom-func($args...)";
    BUILT_IN(custom_func)
    {
      if (ctx.custom_function == NULL) {
        return new (ctx.mem) Null(path, line);
      }
      List* args = ARG("$args", List);
      Inspect i;
      args->perform(&i);
      // std::cout << i.get_buffer() << std::endl;
      string sig_name(sig);
      int pos = sig_name.find_first_of("(",0);
      sig_name = sig_name.substr(0, pos);

      const char* c_result = ctx.custom_function(sig_name.c_str(), i.get_buffer().c_str());
      if (!c_result) {
        return new (ctx.mem) Null(path, line);
      }
      string result_str(c_result);
      delete[] c_result;
      result_str.append(";");
      const char* c_str = result_str.c_str();
      Parser p = Parser::from_c_str(c_str, ctx, c_str, 0);

      Expression* val = p.parse_list();
      Sass::Env tge;
      Sass::Backtrace bt(0, "", 0, "");
      Sass::Eval eval(ctx, &tge, &bt);
      val = val->perform(&eval);
