...
namespace Sass {
    ...
    colors_to_names (map<int, string>())
  {
    ...
    (*this).custom_function_names.clear();
  }
  ...
  void register_built_in_functions(Context& ctx, Env* env)
  {
    ...
    register_function(ctx, print_sig, print, env);

    // custom_function
    for (size_t i = 0, S = ctx.custom_function_names.size(); i < S; ++i) {
      // cout << "custom_function: " << ctx.custom_function_names[i] << endl;
      ctx.custom_function_names[i].append("($args...)");
      register_function(ctx, ctx.custom_function_names[i].c_str(), custom_func, env);
    }
